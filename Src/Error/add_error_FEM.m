function [] = add_error_FEM(Lx,Ly,delx,dely,e0,er,V1,V2,u_FEM_BEM,t_FEM_BEM,folder_name,SALVATAGGIO,margin_BEM_coeff,margini_list,x_r, y_r,outb_r, ptop_r, pbottom_r,TIME_mesh_bem_generation)
    ...
    fprintf('%-15s %-10s %-12s %-10s\n', ...
        '---------', '-------', '------', '--------');
    for k = 1:n_test
        fprintf('%-15.0f %-10d %-12.4g %-10.3f\n', ...
            margini_label(k), N_nodes(k), err_V(k)*100, t_fem_solve(k)+t_fem_mesh(k)); %err_C(k)*100
    end
    fprintf('%-15s %-10d %-12s %-10.3f\n', ...
        'FEM-BEM(ref)', length(x_r), '--', t_FEM_BEM+TIME_mesh_bem_generation);
